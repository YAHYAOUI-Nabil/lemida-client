...
import {
  ...
  Typography, Select, MenuItem, Box
} from '@mui/material';
import { Delete, CheckCircle, HourglassEmpty, Cancel } from '@mui/icons-material';
...
const Commandes = () => {
  ...
  const fetchCommandes = async () => {
    try {
      const response = await axios.get('http://51.83.69.195:6000/api/commandes'); // URL complète pour éviter des problèmes de proxy
  ...
  const handleStatusChange = async (id, status) => {
    try {
        const response = await axios.patch(`http://51.83.69.195:6000/commandes/${id}`, { livraison: status });
  ...
  const renderStatus = (status) => {
    switch (status) {
      case 'Livré':
        return <Typography color="green"><CheckCircle /> Livré</Typography>;
      case 'En cours':
        return <Typography color="orange"><HourglassEmpty /> En cours</Typography>;
      case 'Non livré':
        return <Typography color="red"><Cancel /> Non livré</Typography>;
      default:
        return status;
    }
  };
  ...
  {Array.isArray(commandes) && commandes.length > 0 ? (
    commandes.map(commande => (
      <TableRow key={commande._id}>
        ...
        <TableCell>{commande.nom} {commande.prenom}</TableCell>
        ...
        <TableCell>
  {Array.isArray(commande.cartItems) && commande.cartItems.length > 0 ? (
    commande.cartItems.map((item, index) => (
      <Box key={item._id || index}>
        <Typography variant="body2">{item.titre} quantité: {item.quantity}</Typography>
        <Typography variant="caption">Total : {item.total} €</Typography>
      </Box>
    ))
  ) : (
  ...
  {new Date(commande.date).toLocaleString('fr-FR', {
    ...
    second: '2-digit',
        ...
        <TableCell>
          <IconButton color="secondary" onClick={() => handleDelete(commande._id)}>
            <Delete />
          </IconButton>
        </TableCell>
      </TableRow>
