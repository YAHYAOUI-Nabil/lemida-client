...
const ProduitAdmin = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Voulez-vous vraiment supprimer cet article ?')) {
      try {
        const response = await fetch(`http://51.83.69.195:6000/articles/${id}`, { method: 'DELETE' });
  ...
  return (
    <div className="py-10">
      <h2 className="text-3xl text-center text-green-700 font-bold mb-4">Nos produits</h2>
      {cards.length > 0 ? (
        <div className="pl-4 sm:pl-10 md:pl-20 lg:pl-32 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 justify-items-center">
          {cards.map((card, index) => (
            <div key={index} className="card border p-4 shadow rounded w-full max-w-xs">
              ...
              <p className="text-base font-semibold mt-4 text-gray-700">{card.description}</p>
              <p className="text-green-500 text-2xl font-bold mt-2">{card.prix} €</p>
              <p className="text-base font-semibold mt-4 text-gray-700">Quantité en stock:  {card.quantite} </p>
