...
const CategoriesItem = ({ label, categories, bgColor, categorie }) => {
  ...
  return (
    <div className="px-mobile_padding bg-gray-100 py-4">
      <div className="p-4 bg-white">
        <div className="flex flex-row gap-4 items-end lg:mb-6 mb-4">
          <h1 className="font-bold md:text-xl text-lg text-left">
            <span className="mr-5">{label}</span>
            <NavLink
              to={`/formations/categorie/${categorie}`}
            ...
            {categories.map((category) => {
              return (
                <NavLink
                  ...
                  key={category.name}
                  className={`relative flex items-center justify-center w-52 min-w-52 h-52 rounded-full ${bgColor} cursor-pointer`}
                >
                  <div className="">
